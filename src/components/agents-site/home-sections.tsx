...
import { ConversionCtaBanner, ConversionCtas } from "@/components/agents-site/conversion-ctas";
import { FaqSection } from "@/components/agents-site/faq-section";
import { PricingSection } from "@/components/agents-site/pricing-section";
import { Reveal, Tilt } from "@/components/marketing/motion-primitives";
import { SectionHeader } from "@/components/marketing/section-header";
import { Badge } from "@/components/ui/badge";
import { buttonVariants } from "@/components/ui/button";
import { getAgentIcon } from "@/lib/agents-site/agent-icons";
import {
  AGENT_MARKETPLACE_PREVIEW,
  HOMEPAGE_FAQ,
  HOMEPAGE_TESTIMONIALS,
} from "@/lib/agents-site/catalog";
import { AGENTS_SITE_TAGLINE } from "@/lib/agents-site/constants";
    ...
    <section className="border-t border-border/50">
      <div className="mkt-page">
        ...
        <div className="mt-8 text-center">
          <Link href={href("/mission-control")} className={buttonVariants({ variant: "outline" })}>
    ...
    <section className="border-t border-border/50 bg-muted/10">
      <div className="mkt-page">
        <SectionHeader
          ...
          action={
            <Link href={href("/agents/social-media-agent")} className={buttonVariants({ variant: "outline" })}>
              Browse all agents
            </Link>
          }
        />
          ...
          {AGENT_MARKETPLACE_PREVIEW.map((a, i) => {
            ...
            return (
              <Reveal key={a.slug} delay={i * 0.03}>
                <Link
                  href={href(agentPublicPath(a.slug))}
                  className={cn(
        ...
        <div className="mt-10 grid gap-8 lg:grid-cols-2 lg:items-center">
          ...
          <Reveal delay={0.08}>
            <div className="rounded-2xl border border-primary/20 bg-gradient-to-br from-card/80 to-background/60 p-6 backdrop-blur-xl">
              ...
              <Link href={href("/mission-control")} className={buttonVariants({ className: "mt-6 w-full" })}>
    ...
    <section className="border-t border-border/50 bg-muted/10">
      <div className="mkt-page">
        <SectionHeader
          ...
          action={
            <Link href={href("/skills")} className={buttonVariants({ variant: "outline" })}>
              Open marketplace
            </Link>
          }
        />
